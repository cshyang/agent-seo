...
When the deliverable is a polished client-facing HTML audit (not the plain markdown/Gmail report), the worked example and per-client build script live in the operational scripts directory on the box, not in this repo (they carry client-specific layout and narrative). Copy the example for a new client, swap the data/output paths and dates, and rewrite the per-client narrative prose (diagnosis, roadmap); the section skeleton, CSS, and component patterns (metric cards, callouts, table-wrap, funnel) carry over unchanged.

The one principle that is not negotiable: a fixed, readable house style beats pulling each client's brand palette into dense data tables. A logo and a single accent are the only per-client visual touches — do NOT theme per client. The GEO section within it is data-driven: `build_geo_section(geo)` reads the client's `*-ai-geo-baseline-*.json` and renders the engine x language coverage matrix, the sources-shaping-answers list (search/grounding-infra domains filtered via `NOISE_SOURCE_DOMAINS`/`_is_noise_source`), and the honest presence-not-traffic callout. Renders empty if no GEO data exists.

## GEO baseline pull (generic tool)

A config-driven script in the operational scripts directory (not in this repo) runs the config `geo_prompts` through ChatGPT + Gemini + Perplexity (DataForSEO LLM Responses, live) and writes `data/<client>/<client>-ai-geo-baseline-<date>.json` plus the coverage matrix. It works for any client and replaces the old per-client `*_ai_geo_baseline.py` scripts. Model pins live in the config `geo_model_pins` (currently ChatGPT gpt-5.6-terra, Gemini gemini-3.5-flash, Perplexity sonar) and are held stable for month-over-month comparability; change only via a dated config changelog entry. Note: only ChatGPT accepts geo-localization fields; Gemini/Perplexity are prompt-localized.
