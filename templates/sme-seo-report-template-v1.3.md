...
Version 1.3 · Designed for: GSC + GA4 + DataForSEO pipeline (SERP, Labs, Business Data, On-Page, AI Optimization)
...
| # | Keyword | Language | Theme | Target URL | Baseline pos | Baseline volume |
|---|---------|-------|-----------|--------------|-----------------|
| 1 | `[KW]` | `[EN/MS]` | `[THEME]` | `[URL]` | `[POS]` | `[VOL/mo]` |
...
**Duplicate-event check (mandatory before weighting):** verify no two mapped events fire on the same physical interaction (e.g., a GTM tag and a plugin both tracking one WhatsApp click). Duplicates silently double the weighted number; map ONE canonical event per interaction.
...
### A5. GEO/AI prompt set (10–15 prompts, fixed, language-tagged)
Prompts a real buyer would ask an AI assistant. Same prompts, same engines, every month.

- **Panel size: minimum 10, target 10–15.** Below 10, Section 8 coverage MUST be labeled "insufficient panel — directional only". A 2–4 prompt panel produces anecdotes, not metrics.
- **Language tag every prompt.** In bilingual markets (e.g., EN/MS Malaysia), balance the panel to mirror how buyers actually search each service line; report coverage per language, never blended.
- **Engines: minimum ChatGPT + Gemini + Perplexity**, all via DataForSEO LLM Responses (one pipeline, same parser). Note: Perplexity supports Live method only. Engines cite largely different sources for the same query (reported source-overlap under 0.2), so single-engine tracking materially under-measures. Cost is cents per prompt — engine breadth is cheap; do not economize here.
- Record model, engine, run date, and location context per run. AI responses vary; comparability depends on keeping the panel stable.
- Example prompt shapes: "best `[SERVICE]` company in `[CITY]`", "recommend a `[SERVICE]` firm for `[USE CASE]`", "`[COMPETITOR]` vs alternatives" — plus native-language equivalents written by a native speaker, not translated keyword strings.

| # | Prompt | Language | Engines |
|---|--------|----------|---------|
| 1 | `[PROMPT]` | `[EN/MS/...]` | `[ChatGPT, Gemini, Perplexity]` |

### A6. Changelog
Every change to A2/A3/A5 gets a dated line here. This is what keeps month-12 comparable to month-1.
- 2026-07-21 (v1.3): A5 panel min 10 language-tagged prompts across ChatGPT+Gemini+Perplexity; A2 language column; A3 duplicate-event check; §8 rebuilt as engine×language coverage matrix + sources-shaping-answers; QA additions; llms.txt/schema-as-GEO marked not-used.
...
**Coverage matrix** (the headline — per engine × language, never blended):

| | ChatGPT | Gemini | Perplexity | Total |
|---|---|---|---|---|
| `[LANG_1]` prompts | `[N/N]` | `[N/N]` | `[N/N]` | `[N/N]` |
| `[LANG_2]` prompts | `[N/N]` | `[N/N]` | `[N/N]` | `[N/N]` |
| **Total** | `[N/N]` | `[N/N]` | `[N/N]` | `[N/N]` (last month: `[N/N]`) |
...
- **Sources shaping answers:** `[the 3–6 third-party domains (directories, review sites, portals) the engines cited across the panel, with client present/absent on each]`

>> GEN: (a) Run the fixed A5 panel through DataForSEO LLM Responses — same prompts, same engines, same models where possible, every month; parse response text for client/competitor names and the sources/annotations array for citations. (b) "Sources shaping answers" is the actionable core: GEO leverage is mostly "be present in the sources AI reads," so every absent-from row here should map to a concrete Section 6 listing/placement/entity action, never a vague "improve AI visibility". (c) Distinguish recommendation (AI positions client as a valid option), citation (AI links the page), and mention (named, no link) — they need different fixes. (d) Optional share-of-voice: DataForSEO LLM Mentions (ChatGPT + Google AI Overviews only, 2–7 day index lag — run mid-cycle; coverage of non-English/local prompts is unvalidated, label accordingly). (e) Expectation framing is mandatory: GEO is a presence/recommendation channel, not a traffic channel — even the most-cited brands see under 1% of traffic from AI referrals, and AI platforms misattribute citations at high rates. Never forecast AI referral traffic. (f) A competitor newly appearing across multiple prompts is worth a line.
...
| GEO prompt panel (ChatGPT + Gemini + Perplexity) | DataForSEO LLM Responses (web_search on) | Monthly | Fixed-panel consistency; engines cite different sources, so multi-engine is the coverage floor |
...
Deliberately NOT used: llms.txt as a GEO deliverable (no evidence ChatGPT reads it; not an adopted standard), schema markup sold as a GEO lever (entity hygiene at most — no citation guarantee), Backlinks API (noise for local SMEs; invites unbillable link-building conversations — onboarding toxic-link check at most), Content Generation API (Claude does this), AI Keyword Data / AI search volume (immature metric, revisit when baselines exist), Google Trends endpoints (A1 seasonality notes suffice). Resist adding these back — every added section dilutes the one-diagnosis discipline.
